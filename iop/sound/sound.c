...
#include "sound_log.h"
#include "wmemcpy.h"
#define MODNAME "sound"
IRX_ID("SOUND", 1, 1);

extern struct irx_export_table _exp_sound;

static int transferring = 0;

static int currentPosition;

static int playSample = 0;

static char *currentPlayingAddress;
...
__attribute__((aligned(16))) static unsigned char core1_buffer[4096];

static unsigned short left_buffer[512];
static unsigned short right_buffer[512];
...
static int transferFinished(void *arg)
{
    iSignalSema(transferring);
...
void InitSoundIOP()
{
    if (sceSdInit(0) < 0)
    {
        LOGGER("Cannot initialize libsd");
        return;
    }

    transferring = CreateMutex(0);

    if (transferring < 0)
    {
        LOGGER("Cannot create transferring semaphore");
        return;
    }
    ...
    sceSdSetTransCallback(1, (void*)transferFinished);
    sceSdBlockTrans(1, SD_TRANS_LOOP, core1_buffer, sizeof(core1_buffer), 0);
    ...
    sceSdSetParam(1 | SD_PARAM_BVOLL, 0x3fff);
	sceSdSetParam(1 | SD_PARAM_BVOLR, 0x3fff);
	...
	thr.attr = TH_C;
	thr.thread = play_thread;
	thr.option = 0;
	thr.priority = 39;
	thr.stacksize = 4096;
	play_tid = CreateThread(&thr);
	if (play_tid  < 0)
	{
		LOGGER("Failed to create playing thread");
        return;
	}

	StartThread(play_tid, NULL);

    currentPosition = 0;

    LOGGER("playing thread 0x%x started", play_tid);
...
static void play_thread(void *arg)
{
    int copySize, remaining, intState;
    while (1)
    {
        if (playSample)
        {
            short *src = (short *)(currentPlayingAddress + currentPosition);
            short *left = (short *)left_buffer;
            short *right = (short *)right_buffer;

            if (bufferSize - currentPosition < 512)
            {
                copySize = bufferSize - currentPosition;
                remaining = 512 - copySize;
            } else {
                copySize = 512;
                remaining = 0;
            }
            ...
            currentPosition = currentPosition + (copySize * 2);

            if (currentPosition >= bufferSize)
            {
                src = (short *)(currentPlayingAddress);
            ...
            memset(left_buffer, '\0', sizeof(right_buffer));
        }

        WaitSema(transferring);

        CpuSuspendIntr(&intState);

        int block = 1 - (sceSdBlockTransStatus(1, 0) >> 24);

        unsigned char *buffer_loc = core1_buffer + (block << 11);

        wmemcpy(buffer_loc, left_buffer, 512);
        wmemcpy(buffer_loc + 512, right_buffer, 512);
        wmemcpy(buffer_loc + 1024, left_buffer + 256, 512);
        wmemcpy(buffer_loc + 1536, right_buffer + 256, 512);
        buffer_loc = core1_buffer + (block << 11);

        CpuResumeIntr(intState);
    }
}
...
{
    if (RegisterLibraryEntries(&_exp_sound) != 0)
        return 1;
